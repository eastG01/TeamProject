...
def run_filter(user_id: str, text: str) -> dict:
    ...
    if detected_word:
        final_text = original_text.replace(detected_word, "*" * len(detected_word))
        save_filter_log(user_id, original_text, normalized_text, "욕설사전", None, "악플", "차단")
        penalty = update_user_penalty(user_id)
        return {"result": "악플", "action": "차단", "method": "욕설사전", "ai_score": None, "final_text": final_text, "user_status": penalty["status"]}
    ...
    save_filter_log(user_id, original_text, normalized_text, "KcBERT", ai_score, result, action)
